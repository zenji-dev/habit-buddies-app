...
const MyPartyNet = () => {
    ...
    return (
        <div className="w-full bg-background-dark text-[#25f4f4] neo-border neo-shadow rounded overflow-hidden">
            {/* HEADER SECTION */}
            <div className="flex justify-between items-center p-4 border-b-2 border-[#224949] bg-card-dark">
                <div className="flex items-center gap-3">
                    <div className="w-2 h-2 bg-[#25f4f4] rounded-full"></div>
                    <h2 className="text-lg font-black tracking-tighter uppercase text-white">MY_PARTY_NET</h2>
                </div>
                <div className="flex items-center gap-6">
                    <button className="flex items-center gap-2 px-3 py-1 border-2 border-[#224949] hover:bg-[#25f4f4]/10 hover:border-[#25f4f4] transition-all text-[10px] font-bold rounded">
                        <UserPlus className="w-3 h-3" /> INVITE
                    </button>
                    <div className="text-[10px] font-bold tracking-widest text-[#25f4f4]">
            ...
            {/* INFORMATION LAYER */}
            <div className="p-6 space-y-2 border-b-2 border-[#224949] bg-card-dark">
                ...
                <div className="flex gap-2 text-[11px]">
                    ...
                    <span className="text-[#25f4f4]/80">{partyInfo.description}</span>
                </div>
                <div className="flex gap-2 text-[11px]">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 p-8 items-center border-b-2 border-[#224949]">
                ...
                <div className="flex flex-col items-center justify-center space-y-4">
                    <div className="relative w-48 h-24 overflow-hidden flex items-end justify-center">
                        ...
                        <svg className="absolute top-0 w-48 h-48 -rotate-90">
                            <circle cx="96" cy="96" r="80" fill="none" stroke="#224949" strokeWidth="12" strokeDasharray="251 502" />
                            <circle
                                cx="96" cy="96" r="80" fill="none"
                                stroke={riskMeterValue > 70 ? "#25f4f4" : riskMeterValue > 30 ? "#3a8888" : "#ff2a2a"}
                                strokeWidth="12"
                        ...
                        <div className="z-10 text-center pb-2">
                            <div className="text-[10px] text-[#25f4f4]/40 font-black tracking-[0.2em] uppercase mb-1">Risk Meter</div>
                    ...
                    {participants.map(user => {
                        ...
                        return (
                            <div key={user.id} className="flex flex-col items-center gap-3 group">
                                <div className={`relative w-24 h-24 border-2 p-1 transition-all duration-500 rounded
                  ${isDone ? 'border-[#25f4f4] neo-shadow' : 'border-[#224949]'}`}>

                                    <div className={`w-full h-full bg-background-dark flex items-center justify-center relative overflow-hidden rounded`}>
                                        {user.avatar ? (
                                            <img src={user.avatar} className={`w-full h-full object-cover transition-all ${isDone ? 'opacity-100' : 'grayscale opacity-50'}`} alt="" />
                                        ) : (
                                        ...
                                        {/* Discrete Check Icon */}
                                        {isDone && (
                                            <div className="absolute top-1 right-1 w-5 h-5 bg-[#25f4f4] text-background-dark flex items-center justify-center rounded z-20">
                                                <Check className="w-3.5 h-3.5 stroke-[4px]" />
                                            </div>
                                        )}

                                        {/* Background Blur Overlay for 100% users */}
                                        {isDone && <div className="absolute inset-0 bg-[#25f4f4]/8 z-10 pointer-events-none"></div>}
                                    </div>
                                </div>
                                ...
                                <div className="text-center space-y-1">
                                    ...
                                    <div className="flex justify-center gap-1.5 pt-0.5">
                                        {partyInfo.habits.map((h, idx) => (
                                            <div key={idx} className={`w-1.5 h-1.5 rounded-full ${user.habits[h] ? 'bg-[#25f4f4]' : 'bg-white/5 border border-white/10'}`}></div>
                                        ))}
                                    </div>
                                </div>
                            </div>
            ...
            <div className="w-full overflow-hidden">
                <table className="w-full border-collapse">
                    <thead>
                        <tr className="border-b-2 border-[#224949] text-[9px] uppercase tracking-[0.2em] text-slate-500 text-left bg-[#224949]">
                            <th className="p-4 font-black border-r-2 border-[#224949]">Participant_Node</th>
                            <th className="p-4 font-black text-[#25f4f4]">Daily_Habit_Pulse</th>
                            <th className="p-4 font-black text-right pr-8 text-[#25f4f4]">Efficiency</th>
                        </tr>
                    </thead>
                    <tbody>
                        {participants.map(user => {
                            ...
                            return (
                                <tr key={user.id} className="border-b border-[#224949] hover:bg-[#25f4f4]/[0.03] transition-colors group">
                                    <td className="p-4 w-1/4 border-r-2 border-[#224949]">
                                        <span className="text-[10px] font-bold text-white/80 group-hover:text-[#25f4f4] transition-colors">{user.name}</span>
                                    </td>
                                    <td className="p-4">
                                        <div className="flex flex-wrap gap-4">
                                            {partyInfo.habits.map((habit, ix) => (
                                                <div key={ix} className="flex items-center gap-2">
                                                    <div className={`w-2 h-2 rounded-full ${user.habits[habit] ? 'bg-[#25f4f4]' : 'bg-white/5 border border-white/10'}`}></div>
                                    ...
                                    <td className="p-4 w-1/4">
                                        <div className="flex items-center justify-end gap-3 px-4">
                                            <div className="w-24 h-1 bg-background-dark border border-[#224949] rounded relative">
                                                <div
                                                    className={`h-full transition-all duration-700 rounded ${isDone ? 'bg-[#25f4f4]' : 'bg-[#3a8888]'}`}
                                                    style={{ width: `${pct}%` }}
                                                ></div>
                                            </div>
                                            <span className={`text-[10px] font-black w-8 text-right ${isDone ? 'text-[#25f4f4]' : 'text-slate-400'}`}>{Math.round(pct)}%</span>
                                        </div>
                                    </td>
                                </tr>
                            );
            ...
            {/* FOOTER LEGEND */}
            <div className="p-4 bg-card-dark border-t-2 border-[#224949] flex justify-center gap-8">
                <div className="flex items-center gap-2 text-[8px] font-black tracking-[0.3em] text-white/20">
                    <div className="w-2 h-2 rounded-full bg-[#25f4f4]"></div> COMPLETED_LINK
                </div>
                <div className="flex items-center gap-2 text-[8px] font-black tracking-[0.3em] text-white/20">
                    <div className="w-2 h-2 border border-[#25f4f4]"></div> UPLINK_ACTIVE
                </div>
                <div className="flex items-center gap-2 text-[8px] font-black tracking-[0.3em] text-white/20">
